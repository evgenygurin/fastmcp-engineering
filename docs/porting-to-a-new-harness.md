# Porting FastMCP Engineering to a New Harness

This guide explains how to add support for a new harness — an IDE, CLI, or
agent runner that isn't already covered — so that FastMCP Engineering skills
auto-trigger there the same way they do in the eight current integrations
(Claude Code, Cursor, Copilot CLI, Codex, Kimi Code, OpenCode, pi, Gemini).
...
- Read `AGENTS.md`, `CLAUDE.md`, and `CONTRIBUTING.md` in full — the
...
## Part 1 — How FastMCP Engineering works across harnesses

FastMCP Engineering is the same content everywhere. What changes per harness is
the thin layer that delivers that content to the model and translates its
instructions into the harness's native tools. Three components:
...
2. **Tool mapping (per-harness).** Each harness needs the action vocabulary
   ...
   `skills/using-fastmcp-engineering/references/<harness>-tools.md` and/or inline
   in the harness's bootstrap injector (see Part 5). It says, e.g., "*dispatch a
   subagent* → call `task` with `subagent_type`."

3. **Bootstrap (per-harness).** At the start of every session, the full
   `skills/using-fastmcp-engineering/SKILL.md` is injected into the model's
   context, wrapped in `<EXTREMELY_IMPORTANT>` tags, with the tool mapping
   appended. That injected skill is what teaches the model that skills exist and
   that it must check for a relevant skill before acting. **The bootstrap is the
   entire integration.** Without it, the skill files are inert — present on
   disk, never invoked.
...
A harness can support FastMCP Engineering only if it can do all of the
following. Check these before writing code — if the first one fails, stop.
...
- an **instructions-file** convention where the harness loads a context file that
  *your installed extension ships and declares* (e.g. Gemini's `contextFileName`
  pointing at the extension's own `FME.md`) — not a file you edit in the user's
  home, or
- a **manifest-declared session-start skill** that loads the bootstrap at
  session start (Kimi Code's `sessionStart.skill`), or
- **native skill discovery** where the harness surfaces each installed skill's
  name + description at session start (Codex).

If the only way to get FastMCP Engineering in front of the model is for your
human partner to opt in each session (paste a prompt, run a command, enable a
mode), the harness
...
installer. A harness that can consume an existing manifest via its own `plugin
install` command needs no new files here. Before building, check whether the
harness can simply load an existing manifest. A port that adds nothing to this
repo but a paragraph in the README is a perfectly good outcome.
...
1. The `using-fastmcp-engineering` bootstrap loads at session start, every
   session, with no per-session opt-in.
...
4. **The acceptance test passes.** In a clean session, the user message:
   ...
   > Let's make a FastMCP server

   auto-triggers the `research-first` (or `brainstorming`) skill *before any
   code is written*. Capture the full transcript — the PR requires it.
...
6. A real user can install it through the harness's own mechanism (not by
   ...
   install, so "the *installed* files report the repo version" is not always
   achievable — track the version at the source manifest and don't treat a
   rewritten installed manifest as a failure.

A quick smoke check before the full acceptance test: start a session and ask the
model to describe its fastmcp-engineering skills. If the bootstrap injected, it
knows it has them. (OpenCode's install doc uses `opencode run --print-logs "hello"
2>&1 | grep -i fastmcp` for the same goal via a different mechanism — log-grep
...
**If it's underdocumented, reverse-engineer it empirically:**
...
may expose the parent's manifest fields and `@`-include syntax and *still not
honor them the same way*. Verify with a marker; never assume the parent's recipe
transfers.
...
- Only ever an always-on instructions file, no hook and no code plugin →
  **Shape C**.
- Native skill discovery / manifest session-start skill → **Native** (see the
  routing table).
...
context file, or the harness surfacing the installed `using-fastmcp-engineering`
skill's own description at session start). If more than one install-mechanism
surface injects automatically, prefer the most reliable. What you may **not** do
is bridge a gap by editing the user's global config.
...
`using-fastmcp-engineering/SKILL.md` and prints a JSON object whose **field name
and nesting differ per harness**.
...
> inject context. Confirm the *specific event* you need exists and can write to
> the model's context before committing to Shape A. If it can't, the bootstrap
> belongs in an instructions file (Shape C) instead.
...
- Reference: `.opencode/plugins/fastmcp-engineering.js` (JavaScript) and
  `.pi/extensions/fastmcp-engineering.ts` (TypeScript). pi is the closest
  reference for any harness that has **no native skill tool**.
...
declares* (e.g. Gemini's `contextFileName` → the extension's own `FME.md`).
...
- Reference: `gemini-extension.json` (manifest, with `contextFileName`),
  `FME.md` (two `@`-includes — the bootstrap skill and the tool-mapping
  reference), `skills/using-fastmcp-engineering/references/gemini-tools.md`.
...
### Native skill discovery / manifest session-start skill

Some harnesses surface skills without a session-start hook:

- **Codex** declares `"skills": "./skills/"` with an empty `hooks` object. Codex
  surfaces each installed skill's name + description at session start — the
  `using-fastmcp-engineering` description is what prompts the model to load it.
  Tool mapping lives in `references/codex-tools.md`.
- **Kimi Code** declares `"skills": "./skills/"` plus
  `sessionStart.skill: "using-fastmcp-engineering"` and inline
  `skillInstructions` tool mapping. The manifest itself loads the bootstrap at
  session start.
...
| ships an extension-declared context file it always loads | C (instructions-file) | Gemini (`gemini-extension.json` + `FME.md` + `references/gemini-tools.md`) |
| surfaces skills natively via a manifest `skills` path, no session hook | Native (discovery) | Codex (`.codex-plugin/plugin.json`) |
| has a manifest `sessionStart.skill` field that loads a skill at start | Native (session skill) | Kimi Code (`.kimi-plugin/plugin.json`) |
| has a plugin install command and a manifest `contextFileName` (or equivalent) the installer keeps | C via the plugin installer | (see Part 6 — verify the installer preserves the declared context file) |

Most real harnesses fit one row cleanly; the last is the hybrid case (rule 2
still holds — the bootstrap rides the install mechanism, never a user-config
edit).
...
- **Shape B:** the module the harness loads (e.g. `.<harness>/plugins/*.js`) plus
  ...
  - **Build/dependency check.** Decide how the harness loads your module: does it
    run the source directly (pi's `.ts` is referenced as-is from `package.json`;
    OpenCode ships plain `.js`), or does it need a transpile/build step?
    FastMCP Engineering is zero-runtime-dependency. pi's `import type
...
- **Shape C (instructions-file):** a small manifest (see `gemini-extension.json`:
  ...
  itself (`FME.md` is just two `@`-includes: the bootstrap skill and the
...
`using-fastmcp-engineering` skill content (wrapped in `<EXTREMELY_IMPORTANT>`
tags) plus the harness's tool mapping in front of the model, with a note that the
skill is already active so the model doesn't try to load it again. *How* you do
that — and what you assemble vs. what the harness loads raw — depends entirely on
your shape. Do **not** apply one shape's recipe to another.
...
fastmcp-engineering… for all other skills use the Skill tool" preamble, escapes
it, and prints the harness's JSON shape. The tool mapping for Shape A does **not**
go inline here — it lives in `references/<harness>-tools.md` (Step 4). Get the
JSON output shape exactly right. `hooks/session-start` detects the harness from
environment variables and prints *one of three* shapes:
...
both double-injects). Find the exact field, nesting, and event-matcher values
your harness expects. Then decide: add a fourth branch to `hooks/session-start`,
or — if the harness needs a different bootstrap message or env contract — add a
dedicated `hooks/session-start-<harness>` script. If you add a branch and your
harness *also* sets an env var an earlier branch keys on (some harnesses set
`CLAUDE_PLUGIN_ROOT` too), order your branch before the one that would otherwise
shadow it. Match the harness's own event-matcher strings (Claude Code uses
`startup|clear|compact`, Cursor `sessionStart`); wrong matchers mean the hook
silently never fires.

The **hook-config schema itself varies per harness** — don't assume the Claude
Code shape is universal. Compare `hooks/hooks.json` and
`hooks/hooks-cursor.json`: Cursor's uses `"version": 1`, a lowercase
`sessionStart` key, a relative `./hooks/run-hook.cmd` command, and omits the
`matcher`/`type`/`async` fields Claude Code uses. Match your `hooks-<harness>.json`
to whichever existing file is closest, not to a single canonical template.
...
`hooks-cursor.json` uses a relative path. Use whatever your harness exports. (The
`session-start` script re-derives the root itself via `dirname`, so the script
body doesn't depend on this — but the command in the manifest does.)

**Discovering the harness's contract.** The three facts above — env var, JSON
field/nesting, matcher strings — are the harness's contract, not FastMCP
Engineering's, so you have to source them. Read the harness's hook docs, or find
out empirically: register a throwaway session-start hook that dumps its
environment and emits a marker, then observe which env var identifies the harness
and whether/how the harness ingests your stdout. Pin these down before writing
the real branch.
...
pi's just says "do not try to load using-fastmcp-engineering again." If your
harness has no skill tool, use pi's wording, not OpenCode's.
...
*not* the user's own global file) pulls in two things: the
`using-fastmcp-engineering` skill and the harness's tool-mapping reference.
`FME.md` does this with two `@`-includes (`@./skills/using-fastmcp-engineering/SKILL.md`
and `@./skills/using-fastmcp-engineering/references/<harness>-tools.md`); the
harness loads them raw, frontmatter and all, and `SKILL.md` already carries its
own `<EXTREMELY-IMPORTANT>` block internally. If your harness has no include
syntax, inline the content into the instructions file instead. Gemini ships
**no** "already loaded, don't re-invoke" preamble — for an `@`-include harness the
...
symlink and copies the real files before relying on it). A `skills` path field is
*not* portable.

Where the mapping lives depends on shape:

- **Shape A:** put it in
  `skills/using-fastmcp-engineering/references/<harness>-tools.md`. The agent
  reaches it from the bootstrap — `SKILL.md`'s "Platform Adaptation" section links
  the per-harness references files. (Shape A harnesses have no instructions file;
  the mapping is *not* inlined into the hook output.)
- **Shape B:** the mapping is typically inlined into the bootstrap string you
  inject. pi keeps it in *both* places — `piToolMapping()` inline **and**
  `references/pi-tools.md`. If you maintain it in two places, update both, or the
  port is half-done.
- **Shape C:** put it in `references/<harness>-tools.md` and pull it into the
  always-loaded instructions file (e.g. `FME.md` `@`-includes
  `gemini-tools.md`).
- **Native:** Codex's mapping lives in `references/codex-tools.md`; Kimi's is
  inline in the manifest's `skillInstructions`.
...
`using-fastmcp-engineering/SKILL.md` tells the model to *never read skill files
manually with file tools — always use your platform's skill-loading mechanism.*
The point is "don't bypass the mechanism," not "never use file-read." What counts
as "your platform's mechanism" depends on the harness — and for a harness with no
skill tool, the documented mechanism *is* reading `SKILL.md`. So reading it there
...
2. **Native skill *discovery* but no `Skill` tool** (pi): the harness can find
   and list skills, but the model can't call a tool to load one. Get the skills
   installed where the harness scans (pi registers via `resources_discover` →
   `skillPaths`; OpenCode via its `config` hook), and tell the model to load a
   skill by **reading its `SKILL.md` with the file-read tool when the skill
   applies** — the sanctioned mechanism here, the way `references/pi-tools.md`
   states it.
   ...
   **For the bootstrap itself, prefer a declared context file (Part 6).** If the
   ...
   file through the installer: it's guaranteed-loaded and carries both the
   `using-fastmcp-engineering` content and the tool mapping. That is the strong,
   preferred path.

   **Fallback — the surfaced skill index.** If there's no context-file field but
   the harness surfaces each installed skill's name + description at session
   start, you need *neither* a built index nor a runtime-list instruction — the
   harness is the index, and `using-fastmcp-engineering`'s own surfaced
   description can be what triggers the model to load it. This is softer than a
   declared context file; two things it does **not** give you, versus a context
   file / hook / in-process injector — account for both:
   - **It bootstraps *triggering*, not the *tool mapping*.** An injector prepends
     `<harness>-tools.md` alongside `using-fastmcp-engineering` every session.
     Here nothing injects the mapping — the model only sees skill *descriptions*
     and must *read* your `references/<harness>-tools.md` when it needs tool
     names. It works because skills name actions (the model reads the mapping
     when it acts), but it's softer than injection. Make sure the mapping is
     reachable from what the model loads — e.g. linked from `SKILL.md`'s Platform
     Adaptation section and installed alongside the skills — not just sitting in
     the repo.
   - **There's no structural guarantee the trigger fires.** No
     `<EXTREMELY_IMPORTANT>` wrapper, no dedup, no re-injection after compaction —
     firing depends on the model choosing to act on a description it sees in the
     index. This is exactly why the acceptance test is mandatory here: it is the
     *only* guarantee, so run it on the model(s) your users will actually use, not
...
3. **No skill system at all:** there is nothing to register, and the *only*
   ...
   what it can't find: `using-fastmcp-engineering/SKILL.md` does **not**
   enumerate the available skills, so on its own the model won't know which
   skills exist or their triggers. You must supply a discovery path. Two options,
   and they differ in durability: (a) generate a skill index (each
   `skills/*/SKILL.md`'s `name` + `description` frontmatter) and place it *inside*
   the `<EXTREMELY_IMPORTANT>` wrapper alongside the tool mapping (Shape B recipe
   above) so it's covered by the dedup guard — but a build-time index goes stale
   as skills are added; or (b) instruct the model to list `skills/*/SKILL.md` at
   runtime and read their frontmatter to find a match — slower but never stale.
   Prefer (b) unless you have a reason not to. Without either, a no-skill-system
   port loads the bootstrap but silently never triggers any other skill.
...
- **Shape A:** assert the hook's stdout has the exact JSON shape your harness
  ...
  `tests/hooks/test-session-start.sh`, which validates each harness's output
  shape.
- **Shape B:** a unit test that fakes the harness's plugin API and asserts the
  lifecycle handlers register, the bootstrap injects once, the dedup guard works,
  and (if relevant) compaction re-injection works. See
...
- If the bootstrap is cached, test that the cache behaves when the file is
  missing (see `tests/opencode/test-bootstrap-caching.mjs`).

These automated tests cover the wiring; the live run in Step 7 is what proves the
integration actually triggers skills.
...
auth-gated, or trust-gated. Be ready to do *everything*, including the smoke
check, through tmux.
...
# 3. Smoke check: does the model know it has fastmcp-engineering?
...
tmux send-keys -t port-test 'What are your fastmcp-engineering skills?'; sleep 0.4; tmux send-keys -t port-test Enter
sleep 5
...
tmux send-keys -t port-test 'Let'\''s make a FastMCP server'; sleep 0.4; tmux send-keys -t port-test Enter
# poll until the turn finishes — re-capture every few seconds, don't capture once
sleep 8
tmux capture-pane -t port-test -p          # PASS = research-first triggers BEFORE any code
...
If the smoke check shows the model *doesn't* know it has fastmcp-engineering, the
...
|---|---|---|
| Native plugin marketplace | Claude Code | Register the plugin in a marketplace; users `/plugin install`. Document the marketplace entry in `docs/README.claude-code.md`. |
| External marketplace fork, synced by script | Codex | If the harness needs a fork, sync the tracked plugin files into a separate fork repo and open a PR. Read any include/exclude list so you ship the right tree (it should deliberately drop repo-internal dirs and other harnesses' dotdirs). |
...
| Local installer (plugin install) | (harness-dependent) | A small `install.sh` that runs the harness's own plugin install against a staging dir holding the manifest, the skills, and a generated context file (the bootstrap). Everything arrives through the install mechanism — *not* by editing the user's config (see below). |
...
- **A plugin installer may silently strip *undeclared* files — so make the
  ...
  - **Ship a context file the manifest declares.** If the harness has a
    ...
    live `using-fastmcp-engineering/SKILL.md` + the tool mapping (wrapped in
    ...
    **Verify with a marker** that the installer keeps the file and the harness
    loads it — a porter can wrongly conclude it can't, by shipping the file
    *without* declaring `contextFileName` so it is stripped as unrecognized.
  - **Otherwise lean on the installed `using-fastmcp-engineering` skill itself.**
    If the harness surfaces each installed skill's name + description at session
    start, the `using-fastmcp-engineering` description ("Use when starting any
    FastMCP/MCP engineering work…") can prompt the model to load it — installing
    the skill *is* the bootstrap. Softer (no guaranteed wrapper; it carries
    triggering but not the tool mapping — see Step 5), so prefer the declared
    context file when available.
...
- **Write install docs.** A `docs/README.<harness>.md` (see `docs/README.opencode.md`)
  plus an install section in the top-level `README.md`. The only supported install
  action is **running the harness's own install command** (`gemini extensions
  install`, `/plugin install`, `/add-plugin`, `/plugins install`, etc.).
  Hand-copying skill files and editing the user's global/personal config are
  *both* off-limits (rule 2 / the PR rules). If the harness has no install command
  at all — its only surface is a user-owned config file — then it fails the
  "deliver via install mechanism" rule, and you should raise that rather than
  ship an installer that edits the user's files.
...
- **If no existing channel fits, you're standing up a new one.** None of the rows
  may match your harness. If it needs an external fork sync, clone the Codex sync
  script as the template (note its anchored include/exclude list and its PR
  automation). And whenever you add a new per-harness directory, add it to the
  *other* harnesses' sync excludes so your dotdir doesn't leak into their
  distributions.
...
- Target the **`main`** branch (via a short-lived `feat/<scope>` branch per the
  repo lifecycle). One harness per PR.
- Fill in the PR description and paste the complete acceptance-test transcript
  (the "Let's make a FastMCP server" session showing `research-first`/`brainstorming`
  auto-triggering). A PR without this proof will be closed.
- FastMCP Engineering is a zero-dependency plugin. Don't add a third-party
  runtime dependency. Adding a new harness is the one carve-out the contributor
  rules allow, and even then keep it to what the integration strictly requires —
...
| Cursor | `.cursor-plugin/plugin.json` + `hooks/hooks-cursor.json` | shell hook → `hooks/session-start` (`additional_context`) | none needed (Claude Code–compatible tool surface) | `tests/hooks/` | `/add-plugin` / marketplace |
| Copilot CLI | (shares Claude Code hook path; `COPILOT_CLI` env) | shell hook → `hooks/session-start` (`additionalContext`) | none needed (Claude Code–compatible tool surface) | `tests/hooks/` | plugin install (git URL) |
| Codex | `.codex-plugin/plugin.json` (declares empty `hooks`) | native skill discovery (no session-start hook) | `references/codex-tools.md` | `tests/codex/` | `/plugins` marketplace or git URL |
| Kimi Code | `.kimi-plugin/plugin.json` | manifest `sessionStart.skill` loads `using-fastmcp-engineering` | inline `skillInstructions` in manifest | `tests/kimi/` | marketplace or `/plugins install` GitHub URL |
| OpenCode | `.opencode/plugins/fastmcp-engineering.js` (declared via root `package.json` `main`) | in-process: `config` hook registers skills dir; `experimental.chat.messages.transform` injects user message | inline in `fastmcp-engineering.js` | `tests/opencode/` | `opencode.json` plugin git URL |
| pi | `.pi/extensions/fastmcp-engineering.ts` | in-process: `resources_discover` registers skills; `context` event injects user message; lifecycle-flag + compaction-aware | `piToolMapping()` inline **and** `references/pi-tools.md` | `tests/pi/` | repo-root `package.json` fields |
| Gemini CLI | `gemini-extension.json` + `FME.md` | instructions file `@`-includes bootstrap + mapping | `references/gemini-tools.md` | `tests/gemini/` | `gemini extensions install` |

## Appendix B — Gotchas that have bitten porters

- **Opt-in isn't a port.** If your human partner has to do anything per session
  to get FastMCP Engineering, the acceptance test fails. Re-read Part 2.
...
- **Plugin-root env var differs per harness.** Shape A. The hook command uses
  `${CLAUDE_PLUGIN_ROOT}` (Claude) or a relative path (Cursor). Use what your
  harness exports; the script re-derives the root itself.
